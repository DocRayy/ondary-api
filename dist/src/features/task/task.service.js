...
const pdf_helper_1 = require("../../cores/helpers/pdf-helper");
...
let TaskService = class TaskService {
    ...
    realtimeService;
    eventEmitter;
    constructor(taskRepository, taskTodosRepository, timelogsRepository, userRepository, projectRepository, dataSource, realtimeService, eventEmitter) {
    ...
    async create(payload, currentUserId) {
        const status = payload.status ?? 'draft';
        const orderIndex = payload.order_index ??
            (await this.getNextOrderIndex(payload.project_id, payload.board_column ?? status));
        ...
        const taskWithAssignees = (0, api_response_util_1.responseData)(await this.findOne(task.id));
        this.realtimeService.emitToProject(task.project_id, 'task.created', taskWithAssignees);
        this.eventEmitter.emit(notification_service_1.TASK_CREATED_EVENT, {
            task_id: task.id,
        });
        return (0, api_response_util_1.successResponse)('Task Created', 'Task created successfully', (0, media_url_util_1.withPhotoUrl)((0, remove_passwords_util_1.removePasswords)(taskWithAssignees)));
    }
    async findAll(filters = {}) {
        ...
        if (filters.month || filters.year) {
            const reportPeriod = (0, pdf_helper_1.normalizeReportPeriod)(filters.month?.toString(), filters.year?.toString());
            query.andWhere(`(
        ...
        return (0, api_response_util_1.successResponse)('Tasks Retrieved', 'Tasks retrieved successfully', (0, media_url_util_1.withPhotoUrl)((0, remove_passwords_util_1.removePasswords)(await this.attachAssigneeUsers(await query.getMany()))));
    }
    async findOne(id) {
        ...
        if (!task) {
            throw new common_1.NotFoundException((0, api_response_util_1.errorResponse)('Task Not Found', `Task ${id} was not found`));
        }
        return (0, api_response_util_1.successResponse)('Task Retrieved', 'Task retrieved successfully', (0, media_url_util_1.withPhotoUrl)((0, remove_passwords_util_1.removePasswords)(await this.attachAssigneeUsers(task))));
    }
    async generateTaskReport(res, filters) {
        const reportPeriod = this.normalizePdfReportPeriod(filters.month, filters.year);
        const requestedUserId = Number(filters.user_id);
        const canReadAllReports = this.canReadAllReports(filters.current_user);
        const userId = canReadAllReports &&
        ...
        const taskTodos = await this.taskTodosRepository
            ...
            .leftJoinAndSelect('task.project', 'project')
            .leftJoinAndSelect('todo.timelogs', 'timelog', `(
          (
        ...
        if (type) {
            taskTodos.andWhere('(todo.status = :type OR task.status = :type OR task.board_column = :type)', { type });
        }
        if (hasUserFilter) {
            taskTodos.andWhere(`(
          todo.user_id = :userId
        ...
        const data = this.buildTaskReportData(await taskTodos.getMany(), reportPeriod, await timelogs.getMany(), await this.buildReportFilterSummary({
            reportPeriod,
        ...
        return (0, pdf_helper_1.generatePdf)(res, data, filename);
    }
    async update(id, payload) {
        const currentTask = (0, api_response_util_1.responseData)(await this.findOne(id));
        const task = await this.taskRepository.preload({
            id,
            ...payload,
            assignee_user_ids: payload.assignee_user_ids === undefined
                ? undefined
        ...
        this.realtimeService.emitToProject(savedTask.project_id, 'task.updated', taskWithAssignees);
        return (0, api_response_util_1.successResponse)('Task Updated', 'Task updated successfully', (0, media_url_util_1.withPhotoUrl)((0, remove_passwords_util_1.removePasswords)(taskWithAssignees)));
    }
    async move(id, payload, movedBy) {
        const task = (0, api_response_util_1.responseData)(await this.findOne(id));
        const movedAt = new Date();
        const movedTask = await this.dataSource.transaction(async (manager) => {
            const taskRepository = manager.getRepository(entities_1.TaskEntity);
            await taskRepository
                .createQueryBuilder()
                .update(entities_1.TaskEntity)
                ...
                .execute();
            const completedAt = payload.status === 'completed' ? (task.completed_at ?? movedAt) : null;
        ...
        this.realtimeService.emitToProject(movedTask.project_id, 'dashboard.refresh', { project_id: movedTask.project_id });
        return (0, api_response_util_1.successResponse)('Task Moved', 'Task moved successfully', (0, media_url_util_1.withPhotoUrl)((0, remove_passwords_util_1.removePasswords)(movedTaskWithAssignees)));
    }
    async remove(id) {
        const task = (0, api_response_util_1.responseData)(await this.findOne(id));
        ...
        return (0, api_response_util_1.successResponse)('Task Deleted', 'Task deleted successfully');
    }
    async getNextOrderIndex(projectId, boardColumn) {
    ...
    emitStatusUpdated(currentTask, savedTask, updatedBy) {
        if (currentTask.status === savedTask.status) {
            return;
        }
        this.eventEmitter.emit(notification_service_1.TASK_STATUS_UPDATED_EVENT, {
    ...
    canReadAllReports(user) {
        return user.role === 'manager' || user.role === 'admin';
    }
    normalizePdfReportPeriod(month, year) {
        const parsedMonth = Number(month);
        const parsedYear = Number(year);
        const hasMonthFilter = month !== undefined &&
            month !== '' &&
            !['all', 'all_months', 'all-months', 'all months'].includes(month.trim().toLowerCase()) &&
            Number.isInteger(parsedMonth) &&
            parsedMonth >= 1 &&
            parsedMonth <= 12;
        const selectedYear = Number.isInteger(parsedYear) && parsedYear > 1900
            ? parsedYear
            : new Date().getFullYear();
        if (hasMonthFilter) {
            return {
                ...(0, pdf_helper_1.normalizeReportPeriod)(String(parsedMonth), String(selectedYear)),
                hasMonthFilter: true,
            };
        }
    ...
    async attachAssigneeUsers(taskOrTasks) {
        const tasks = Array.isArray(taskOrTasks) ? taskOrTasks : [taskOrTasks];
        const assigneeIds = [
            ...new Set(tasks.flatMap((task) => this.normalizeUserIds(task.assignee_user_ids))),
        ];
        const users = assigneeIds.length > 0
            ? await this.userRepository.find({
                where: { id: (0, typeorm_2.In)(assigneeIds) },
            })
            : [];
    ...
    buildTaskReportData(taskTodos, reportPeriod, timelogs = [], filterSummary) {
        const rows = [
            ...taskTodos.map((todo) => this.buildTodoReportRow(todo, reportPeriod.startDate, reportPeriod.endDate)),
            ...timelogs.map((timelog) => this.buildTimelogReportRow(timelog, reportPeriod.startDate, reportPeriod.endDate)),
        ].sort((first, second) => first.reportDate.getTime() - second.reportDate.getTime());
        const reportRows = rows.map(({ reportDate: _reportDate, ...row }) => row);
        const projectNames = new Set(reportRows.map((row) => row.project).filter((project) => project !== '-'));
        const totalTimeSpendMinutes = reportRows.reduce((total, row) => total + row.timeSpendMinutes, 0);
        const totalTodos = reportRows.length;
        const totalProjects = projectNames.size;
        const totalCompleted = reportRows.reduce((total, row) => total + row.completed, 0);
        const performance = (0, pdf_helper_1.calculateTaskReportPerformance)({
    ...
    async buildReportFilterSummary(filters) {
    ...
    buildTodoReportRow(todo, startDate, endDate) {
        const timeSpendMinutes = (todo.timelogs ?? []).reduce((total, timelog) => total + this.getTimelogMinutes(timelog), 0);
        ...
        const isCompleted = this.isDateInRange(completedDate, startDate, endDate) &&
            (todo.progress >= 100 ||
                [
                    'finish',
                    'finished',
    ...
    getTodoAssigneeLabel(todo) {
        const assignedUsers = todo.todoUsers
    ...
    buildTimelogReportRow(timelog, startDate, endDate) {
        ...
        return {
            ...
            created: this.isDateInRange(timelog.created_at, startDate, endDate)
                ? 1
                : 0,
            completed: this.isDateInRange(timelog.end, startDate, endDate) ||
    ...
    getTimelogMinutes(timelog) {
        ...
        if (timelog.start && timelog.end) {
            return Math.max(0, Math.floor((timelog.end.getTime() - timelog.start.getTime()) / 60000));
        }
        return 0;
    }
    ...
    getReportRowDate(todo, startDate, endDate) {
        ...
        const timelogDate = (todo.timelogs ?? [])
            ...
            .filter((date) => this.isDateInRange(date, startDate, endDate))
    ...
    getTimelogReportDate(timelog, startDate, endDate) {
        return ([timelog.start, timelog.end, timelog.created_at, timelog.updated_at]
            .filter((date) => this.isDateInRange(date, startDate, endDate))
            .sort((first, second) => first.getTime() - second.getTime())[0] ??
            timelog.created_at);
    }
    ...
    formatReportMonth(date) {
        return date.toLocaleDateString('en-US', {
            month: 'long',
            year: 'numeric',
        });
    }
    formatReportDay(date) {
        return date.toLocaleDateString('en-US', {
            weekday: 'long',
            day: '2-digit',
            month: 'short',
...
exports.TaskService = TaskService = __decorate([
    ...
    __param(2, (0, typeorm_1.InjectRepository)(entities_1.TimelogEntity)),
