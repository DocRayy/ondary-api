...
exports.normalizeReportMonth = normalizeReportMonth;
exports.normalizeReportPeriod = normalizeReportPeriod;
exports.calculateTaskReportPerformance = calculateTaskReportPerformance;
exports.formatReportDuration = formatReportDuration;
exports.taskGeneratehtml = taskGeneratehtml;
exports.generatePdf = generatePdf;
const pdfkit_1 = __importDefault(require("pdfkit"));
function normalizeReportMonth(month, year) {
    const now = new Date();
...
function calculateTaskReportPerformance(data) {
    if (data.totalTimeSpendMinutes <= 0 &&
        data.totalTodos <= 0 &&
    ...
    const completionRate = data.totalTodos > 0 ? data.totalCompleted / data.totalTodos : 0;
    ...
    const score = Math.round(timeScore + todoScore + projectScore + completionScore);
    if (score >= 80) {
        return {
...
function formatReportDuration(totalMinutes) {
    const minutes = Math.max(0, Math.floor(totalMinutes || 0));
    const hours = Math.floor(minutes / 60);
    const remainingMinutes = minutes % 60;
    return `${hours}h ${remainingMinutes}m`;
}
function taskGeneratehtml(data) {
...
async function generatePdf(res, data, filename = 'task-report.pdf') {
...
function drawSummary(doc, data, margin, cardColor) {
    const cardY = 20;
    ...
    doc
        ...
        .fontSize(17)
        .text(formatReportDuration(data.totalTimeSpendMinutes), margin + 75, cardY + 31)
        .fontSize(8)
    ...
    doc
        ...
        .font('Helvetica-Bold')
        .fontSize(17)
        .text(titleParts.slice(0, -1).join(' ') || data.performanceTitle, thirdX + 95, cardY + 12, {
        width: thirdWidth - 105,
    })
        .text(titleParts.length > 1 ? titleParts[titleParts.length - 1] : '', thirdX + 95, cardY + 32, {
        width: thirdWidth - 105,
    })
        .fontSize(7)
        .font('Helvetica')
        .text(`${data.performanceSubtitle} (${data.performanceScore})`, thirdX + 95, cardY + 51, {
        width: thirdWidth - 105,
    });
    doc
        .lineWidth(2)
        .path(`M${thirdX + 65},${cardY + 18} C${thirdX + 50},${cardY + 28} ${thirdX + 58},${cardY + 45} ${thirdX + 73},${cardY + 43} C${thirdX + 88},${cardY + 41} ${thirdX + 92},${cardY + 23} ${thirdX + 80},${cardY + 16} C${thirdX + 82},${cardY + 24} ${thirdX + 73},${cardY + 27} ${thirdX + 70},${cardY + 20} C${thirdX + 69},${cardY + 17} ${thirdX + 67},${cardY + 15} ${thirdX + 65},${cardY + 18} Z`)
        .stroke('#000000');
}
function drawTable(doc, data, margin, startY, textColor) {
    const columns = [
    ...
    data.rows.forEach((row, index) => {
        ...
        const shouldDrawGroup = !previousRow ||
